...
data:extend({
  ...
  }, 
  {
    ...
    expression = "5000 * max(starting, gleba_simple_spot(1000, 6 * size ^ 0.5, 80 / frequency ^ 0.5, gleba_highland) * max(gleba_highland, gleba_midland_aux_2)) * richness / size",
    local_expressions =
    {
      richness = "control:gleba_lead:richness",
      frequency = "control:gleba_lead:frequency",
      size = "control:gleba_lead:size",
      starting = "starting_spot_at_angle{ angle = gleba_starting_angle + 40 * gleba_starting_direction,\z
                                          distance = 60 * gleba_starting_area_multiplier,\z
                                          radius = 7 * size ^ 0.5,\z
                                          x_distortion = gleba_wobble_x * 8,\z
                                          y_distortion = gleba_wobble_x * 8}"
